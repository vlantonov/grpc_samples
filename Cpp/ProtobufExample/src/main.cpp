...
void saveHero(const char* fname, const Hero& hero) {
  ...
  if (!hero.SerializeToOstream(&out)) throw runtime_error("saveHero() failed");
}

void loadHero(const char* fname, Hero& hero) {
  fstream in(fname, ios::in | ios::binary);
  if (!hero.ParseFromIstream(&in)) throw runtime_error("loadHero() failed");
}
...
void printHero(const Hero& hero) {
  ...
  if (hero.has_mage_info()) {
    ...
    cout << "Weapon: "
         << (hero.warrior_info().weapon() == Weapon::SWORD
                 ? "sword"
                 : hero.warrior_info().weapon() == Weapon::BOW
                       ? "bow"
                       : "(unknown weapon)")
         << endl;
